...
import time as time_to_stop_the_game
...
# Первоначальные настройки
    def __init__(self):
        ...
        # Для поиска фигур на диогоналях
        self.all_lines = [
            [[6,0],[7,1]],
            [[4,0],[5,1],[6,2],[7,3]],
        ...
        self.bot_row = None
        self.bot_col = None
        self.bot_result_pos = None
        self.bot_thread = None
        self.bot_moved.connect(lambda:self.on_bot_move_completed())
        self.is_main_closed = False
    ...
    # Отображение времени для таймера при игре с другом
    def showTime(self):  
    ...
    # Отображение последнего состояния доски (до перемотки)  
    ...
    # Поиск всевозможных ходов для конкрентой фигуры
    ...
    # Позиции с которых можно совершить сбитие вражеских фигур
    ...
    # Поиск фигур на одной диогонали
    ...
    # Проверка фигуры непревратилась ли она в дамку
    ...
    # Получение имени кнопки по ее координатам
    def get_name_by_row_and_col(self, row, col):
        return f"button_{row}{col}"
    
    # Проверка фигуры на королеву
    ...
    def choose_start_position(self, button = None, index_r = None, index_c = None):
        ...
        if (not self.is_game_with_bot and figure_type == self.type_of_figure) or (self.is_game_with_bot and figure_type=="white"): 
    ...
    # Поиск позиций, для сбития фигур
    ...
    # Удаление сбитых фигур по позиции
    ...
    # Удаление сбитых фигур из списка
    ...
    def make_notes_about_figure_which_make_the_motion(self, row_parent, col_parent, new_row, new_col, type_of_figure, time=False):
        ...
        if type_of_figure == "black":
            ...
            if self.is_game_with_bot:
                new_text = text + f"\n\n    {self.count_of_black_motions}. {dict_of_letters_to_numbers[col_parent]}{row_parent} - {dict_of_letters_to_numbers[new_col]}{new_row}\t "
            ...
            if self.is_game_with_bot:
                new_text = text + f"\n\n    {self.count_of_white_motions}. {dict_of_letters_to_numbers[col_parent]}{row_parent} - {dict_of_letters_to_numbers[new_col]}{new_row}\t "
                self.ui.textEdit_of_white_motions.setText(new_text)
            else:
                new_text = text + f"\n\n    {self.count_of_white_motions}. {dict_of_letters_to_numbers[col_parent]}{row_parent} - {dict_of_letters_to_numbers[new_col]}{new_row}\t {time}c  "
                self.ui.textEdit_of_white_motions.setText(new_text)
    ...
    # Печать доски, терминальная функция
    ...
    # Ход и различные действия с GUI
        ...
        if figure_type == None and [index_of_row, index_of_col] in ways:
            ...
            if not self.is_game_with_bot:
                ...
                if self.type_of_figure=="black":
                    ...
                    self.ui.label_time.setText("Бот думает...")
            ...
            if self.is_game_with_bot:
                QTimer.singleShot(0, lambda:self.play_checker_with_bot())
                    
    # разработка бота

    # Поиск ходов (приоритет взятия)
    def get_ways_to_cut_other(self, color):
        list_of_ways = []
        list_of_more_important_ways = []
        visited_list_of_ways = []
        visited_list_of_more_important_ways = []
        for i in range(8):
            for j in range(8):
                if self.board[i][j].type_of_figure == color:
                    ways= []
                    more_important_ways = []
                    self.get_all_ways(i,j, ways, more_important_ways, self.board[i][j].isqeen, color)
                    if len(more_important_ways)!=0:
                        for pos in more_important_ways:
                            if [i, j, pos[0], pos[1]] not in visited_list_of_more_important_ways:
                                move = Move()
                                move.get_el(i,j,pos[0], pos[1])
                                list_of_more_important_ways.append(move)
                                visited_list_of_more_important_ways.append([i,j,pos[0], pos[1]])
                    if len(ways)!=0:
                        for pos in ways:
                            if [i,j,pos[0], pos[1]] not in visited_list_of_ways:
                                move = Move()
                                move.get_el(i,j,pos[0], pos[1])
                                list_of_ways.append(move)
                                visited_list_of_ways.append([i,j,pos[0], pos[1]])
        if len(list_of_more_important_ways)>0:
            return list_of_more_important_ways, list_of_ways
        else:
            return list_of_ways, list_of_more_important_ways

    # Поиск ходов из конкретной позиции 
    ...
    #     bonus_for_most_central = 0.3
    #     bonus_for_more_central = 0.2
    #     bonus_for_qeen = 1.0          
    #     bonus_for_mobility = 0.1
    #     bonus_for_mobility_cut = 0.3  
    #     bonus_to_play_qeen = 0.5      
    #     transformation_threat = 1.5 
    ...
    #             #     if not self.board[i][j].isqeen and i == 1:
    #             #         black_positional_bonus+=transformation_threat
    #             # elif self.board[i][j].type_of_figure=="white":
    #             #     if self.board[i][j].isqeen:
    #             #         white_positional_bonus+=bonus_to_play_qeen
    #             #     if not self.board[i][j].isqeen and i ==6:
    #             #         white_positional_bonus+=transformation_threat
    ...
    #                     if j in list_of_center_row_and_col:
    #                         white_positional_bonus+= bonus_for_most_central
    ...
    #                     ways = []
    #                     more_important_ways = []
    #                     self.get_all_ways(i,j,ways, more_important_ways, self.board[i][j].isqeen, "black")
    #                     black_positional_bonus+= len(ways)*bonus_to_play_qeen
    ...
    # def check_position(self):
    #     BONUS_FOR_QUEEN = 2.0
    #     BONUS_FOR_CENTER = 2.5
    #     BONUS_FOR_MOBILITY = 2.5
    #     BONUS_FROM_THREAT_TRANSFORMATION = 15
    #     ENSHPIL_BONUS = 2.0
    #     BONUS_DEFENCE = 3.0
    #     total_bonus = 0
    #     for row in range(8):
    ...
    #             if piece.type_of_figure =="black":
    #                 ways = []
    #                 more_important_ways = []
    #                 self.get_all_ways(row, col, ways, more_important_ways, piece.isqeen, piece.type_of_figure)
    #                 total_bonus+=len(ways)*BONUS_FOR_MOBILITY
    #                 if col in [0,7]:
    #                     total_bonus+= BONUS_DEFENCE
    #                 if row in [1,2]:
    #                     total_bonus+=BONUS_FOR_QUEEN
    #                 if row in list_of_center_row_and_col and col in list_of_center_row_and_col:
    #                     total_bonus+=BONUS_FOR_CENTER
    #                 if row == 7 and not piece.isqeen and self.count_of_black_figure>7:
    #                     total_bonus+=ENSHPIL_BONUS
    #                 if not piece.isqeen and row == 6:
    #                     total_bonus+= BONUS_FROM_THREAT_TRANSFORMATION
    #             elif piece.type_of_figure == "white":
    #                 if not piece.isqeen and row ==1:
    #                     total_bonus-= BONUS_FROM_THREAT_TRANSFORMATION*2                    
    #                     # total_bonus-= len(more_important_ways)*NEGATIVE_BONUS_FOR_QEEN*3
    #                     # total_bonus-= len(ways)*NEGATIVE_BONUS_FOR_QEEN
    ...
    #     black_material = 0
    #     white_material = 0
    #     position_bonus = 0  # Бонус за позицию
        
    #     for i in range(8):
    #         for j in range(8):
    #             piece = self.board[i][j]
    #             if piece.type_of_figure == "black":
    #                 value = king_value if piece.isqeen else piece_value
    #                 black_material += value
                    
    #                 # Бонус за продвижение вперед (для простых шашек)
    #                 if not piece.isqeen:
    #                     position_bonus += (7 - i) * 5  # Чем ближе к дамочному полю, тем лучше
    ...
    #                 if not piece.isqeen:
    #                     position_bonus -= i * 5  # Для белых чем ниже, тем лучше
        
    #     # Учитываем позиционный бонус в оценке
    #     return (black_material - white_material) + position_bonus

                            
    # def evaluate(self):
    # # Материал (базовые веса)
    #     piece_value =300
    #     king_value = 900
    ...
    #                     count_of_white_figure+=1
    #     material = (
    #        (count_of_black_figure-count_of_white_figure)*piece_value+ (count_of_black_qeen - count_of_white_qeen)*king_value
    #     )
        
    #     positional = self.check_position()
        
    #     return material + positional

    # Проверка на защищенность фигуры
    def is_figure_protected(self, row, col, color):
        piece = self.board[row][col]
        if color == "black":
            if row +1<=7:
                if col-1>=0 and col+1>7 and self.board[row+1][col-1].type_of_figure ==color:
                    return True
                elif col+1<=7 and col-1 < 0 and self.board[row+1][col+1].type_of_figure == color:
                    return True
                elif col-1>=0 and col+1 <=7 and self.board[row+1][col-1].type_of_figure == color and self.board[row+1][col+1].type_of_figure == color:
                    return True
                else:
                    return False
            else:
    ...
    # Функция оценивания позиции
    def evaluate(self):
        figure_value = 1.0
        king_value = 4.0
        bonus_protected = 0.15
        negative_bonus_for_alone_figure = 0.05
        list_of_black_moves  = self.get_ways_to_cut_other("black")[0]
        list_of_white_moves = self.get_ways_to_cut_other("white")[0]
        score = 0
        for row in range(8):
            for col in range(8):
                piece = self.board[row][col]
                if  piece.type_of_figure == "black":
                    ...
                    if piece.isqeen:
                        ...
                        if col ==0 or col == 7:
                            score -= negative_bonus_for_alone_figure
                        score+=(7 - row) * 0.05
                        score+=figure_value
                    is_protected = self.is_figure_protected(row, col, "black")
                    if is_protected:
                        score+= bonus_protected
                elif piece.type_of_figure == "white":
                    if piece.isqeen:
                        score -= king_value
                    else:
                        ...
                        score-= row * 0.05
                        score -= figure_value
                    is_protected = self.is_figure_protected(row, col, "white")
                    if is_protected:
                        score-= bonus_protected               
        score+= 0.1 *(len(list_of_black_moves)-len(list_of_white_moves))
        return score
    ...
    def minimax(self, is_maxplayer, depth, alpha, beta, row=-1, col=-1):
        ...
        if is_maxplayer:
            ...
            if row!=-1:
                ...
                #     possible_ways = self.get_ways_to_cut_other("black")
            else:
                possible_ways = self.get_ways_to_cut_other("black")[0]

            if not possible_ways:
                return self.evaluate(), None
            # print("\n possible_ways for black ")
            ...
            for way in possible_ways:
                ...
                check_is_qeen = False
                if not old_isqeen and way.row == 0:
                    check_is_qeen = True
                    self.board[way.row][way.col].isqeen = True
                    self.count_of_black_figure-=1
                    self.count_of_black_qeen+=1
                ...
                if lists:
                    ...
                    more_important_ways = []
                    if check_is_qeen:
                        self.get_all_ways(way.row, way.col,ways, more_important_ways, check_is_qeen, old_type)
                    else:
                        self.get_all_ways(way.row, way.col,ways, more_important_ways, old_isqeen, old_type)
                    if more_important_ways:
                        eval, _ = self.minimax(is_maxplayer, depth, alpha, beta, way.row, way.col)
                    else:
                        eval, _ = self.minimax(not is_maxplayer, depth-1, alpha, beta)
            ...
            if row!=-1:
                possible_ways = self.get_possible_ways_using_coordinats(row, col)
                # if not possible_ways:
                #     possible_ways = self.get_ways_to_cut_other("white")
            else:
                possible_ways = self.get_ways_to_cut_other("white")[0]
            if not possible_ways:
                return self.evaluate(), None
            # print("\n possible_ways for white ")
            # for way in possible_ways:
            #     print(way, end="    ")
            for way in possible_ways:
                old_type = self.board[way.parent_row][way.parent_col].type_of_figure
                ...
                self.board[way.parent_row][way.parent_col].isqeen = False
                check_is_qeen = False
                if not old_isqeen and way.row == 7:
                    check_is_qeen = True
                    self.board[way.row][way.col].isqeen = True
                    self.count_of_white_figure-=1
                    self.count_of_white_qeen+=1
    ...
    # Функция для отслеживания, доступно ли главное окно к закрытию
    def make_main_window_clicked_or_unclicked(self, is_clicked):
        self.is_main_closed = is_clicked

    # Функия для закрытия либо игнорирования закрытия главного окна
    ...
    # Функция получающая позицию для минимакса, для работы в другом потоке
    def bot_calculation_thread(self):
        _, self.bot_result_pos = self.minimax(True, self.depth, float('-inf'), float('inf'), self.bot_row, self.bot_col)
        self.bot_moved.emit()

    # Функция координирующая игру с ботом
    def play_checker_with_bot(self, row = -1, col =-1):
        if self.type_of_figure=="black":
            self.make_board_buttons_not_touchble(True)
            # self.make_board_buttons_clicked_or_unklicked(False)
            self.make_main_window_clicked_or_unclicked(True)
            self.ui.stop_game.setAttribute(Qt.WA_TransparentForMouseEvents, True)
            # _, pos = self.minimax(True, self.depth, float('-inf'), float('inf'), row, col)
            self.bot_row = row
            self.bot_col = col
            self.bot_thread = QThread()
            self.bot_thread.run = self.bot_calculation_thread
            self.bot_thread.start()
        # else:
        #     self.make_board_buttons_not_touchble(False)
    
    # Функция непосредственно совершающая ход бота 
    def on_bot_move_completed(self):
        if self.bot_result_pos!=None:
            # print(pos)
            # print(eval)
            self.parent_r =self.bot_result_pos.parent_row
            self.parent_c = self.bot_result_pos.parent_col
            self.choose_start_position(None, self.bot_result_pos.row, self.bot_result_pos.col)
        self.ui.stop_game.setAttribute(Qt.WA_TransparentForMouseEvents, False)
        self.make_main_window_clicked_or_unclicked(False)
    ...
    # def make_color_buttons_clicked_or_unclicked(self, color, is_clicked):
    #     for button in self.all_buttons:
    #         row = int(button.objectName()[-2])
    #         col = int(button.objectName()[-1])
    #         if self.board[row][col].type_of_figure == color:
    #             button.setAttribute(Qt.WA_TransparentForMouseEvents, is_clicked)
    #             # button.setEnabled(is_clicked)
    ...
    def start_game(self):
        ...
        self.ui.button_prev_motion.setIcon(QIcon(resource_path(icon_for_prev_motion)))
        self.ui.button_return_this_position.setIcon(QIcon(resource_path(icon_for_this_motion)))
        self.ui.button_next_motion.setIcon(QIcon(resource_path(icon_for_next_motion)))
        self.ui.push_button_play_game_with_bot.setEnabled(True)
        
    def make_start_buttons_clicked_or_unclicked(self,is_enabled = False):
        self.ui.start_game_2_players.setEnabled(is_enabled)
        self.ui.push_button_play_game_with_bot.setEnabled(is_enabled)
    ...
    def start_game_for_bot(self):
        ...
        self.make_start_buttons_clicked_or_unclicked(False)
        # self.make_color_buttons_clicked_or_unclicked("black", True)
    ...
    def go_to_preview(self):
        ...
        self.temp = self.ui.first_cenral_widget
        self.start_settings()
        self.ui.button_prev_motion.setIcon(QIcon())
        self.ui.button_return_this_position.setIcon(QIcon())
        self.ui.button_next_motion.setIcon(QIcon())
    ...
    def make_board_buttons_not_touchble(self, is_clicked):
        for button in self.all_buttons:
            button.setAttribute(Qt.WA_TransparentForMouseEvents, is_clicked)
